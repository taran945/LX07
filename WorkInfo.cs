...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.actvityName from workinfo join activity on workinfo.activityId=activity.activityId where workId=@id";
        ...
        public IEnumerable<Model.WorkInfo> GetNew()
        {
            using (IDbConnection cn =new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='待审核' order by uploadTime desc limit 8 ";
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='待审核' and reommend='是' order by uploadTime desc limit 6 ";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public int GetFindCount(string workName)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from workinfo where workVerify='待审核' and workName like concat('%',@workName,'%')";
        ...
        public int UpdateImg(Model.UserInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int Delect(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "delect from workinfo where workid=@id";
